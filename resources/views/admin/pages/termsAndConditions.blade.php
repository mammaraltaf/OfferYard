...
@section('content')
    ...
    <div class="card-header pt-5">
        ...
        <h3 class="card-title">
            <span class="card-label fw-bolder fs-3 mb-1">Manage Terms And Conditions</span>
        </h3>
    </div>
    ...
    <div class="card-body py-3">
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h1 class="modal-title">Add Term And Condition</h1>
                    </div>
                    <div class="modal-body">
                        <form method="POST" action="{{ route('admin.termsAndConditionsPost') }}">
                            @csrf
                            ...
                            <br>
                            <label class="control-label">Select Term and Condition Status</label>
                            <div>
                            ...
                            <div class="form-group">
                                <div>
                                    <button type="submit" class="btn btn-success">Add Term and Condition</button>
                                </div>
                            </div>
                            <br>
                        </form>
        ...
        <div id="EditModalForm" class="modal fade">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h1 class="modal-title">Edit Term and Condition</h1>
                    </div>
                    <div class="modal-body">
                        <form method="POST" id="serviceFormEdit" action="">
                            @csrf
                            <input type="hidden" name="termsConditionId" id="termsConditionId">
                            <div class="form-group">
                                <label class="control-label">Enter Title</label>
                                <div>
                            ...
                            <div class="form-group">
                                <div>
                                    <button type="submit" class="btn btn-success">Update Term and Condition</button>
                                </div>
                            </div>
                            <br>
                        </form>
...
{{--                        <td width="100px">{{$termAndCondition->description}}</td>--}}
                        <td><span class="{{$termAndCondition->status == 1 ? 'badge badge-success' : 'badge badge-danger'}}">{{$termAndCondition->status == 1 ? 'Enable' : 'Disable'}}</span></td>
                        <td>
                            <a id="deleteBtn" data-toggle="modal" data-target=".modal1" data-id="{{$termAndCondition->id}}"
                               class="{{$termAndCondition->status == 1 ? '' : 'btn btn-sm btn-success'}} delete_btn">{{$termAndCondition->status == 1 ?  '' : 'Enable'}}</a>

                            <a href="{{url('admin/edit-terms-and-conditions',$termAndCondition->id)}}" id="serviceEdit" class="btn btn-primary btn-sm"
                               data-id="{{$termAndCondition->id}}" id="{{$termAndCondition->id}}" data-toggle="modal" data-target="#EditModalForm">Edit</a>
                        </td>
                    </tr>
                @endforeach
                </tbody>
            ...
            <div class="modal fade modal1" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel">
                <div class="modal-dialog modal-sm" role="document">
                    <div class="modal-content">
                        ...
                        <form method="post" action="{{route('admin.changeTermsAndConditionStatus')}}">
                            @csrf
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('body').on('click', '#serviceEdit', function () {
                ...
                $.ajax({
                    type: "GET",
                    url: "{{url('/admin/edit-terms-and-conditions/')}}"+'/'+termsAndConditions_id,
                    success:function (response){
                        console.log(response);
                        $('#termsConditionId').val(response.id);
                        $('#title').val(response.title);
                        $('#description').val(response.description);
                        $('#status').prop('selectedIndex', response.status);
                        $('#serviceFormEdit').attr('action',"{{url('/admin/edit-terms-and-conditions/')}}"+'/'+termsAndConditions_id);
                    }
                });
            });
        });
