...
@section('pageTitle') Requests @endsection
@section('content')
    <!--begin::Header-->
    <br>
            ...
            <table id="example" name="allTable" class="allTable table table-striped table-bordered dt-responsive nowrap" style="width:100%">
                ...
                @foreach($requests as $request)
                    <tr>
                        ...
                        <td>{{$request->providero->provider_business_name ?? ''}}</td>
                        <td><span class="{{$request->request_status == 1 ? 'badge badge-success' : 'badge badge-warning'}}">{{$request->request_status == 1 ? 'Completed' : 'Pending'}}</span></td>
                    </tr>
                @endforeach
                </tbody>
                <tfoot>
