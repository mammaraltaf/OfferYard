...
@section('content')
    ...
    <div class="card-header pt-5">
        ...
        <h3 class="card-title">
            <span class="card-label fw-bolder fs-3 mb-1">Manage Refund Policy</span>
        </h3>
    </div>
    ...
    <div class="card-body py-3">
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h1 class="modal-title">Add Refund Policy</h1>
                    </div>
                    <div class="modal-body">
                        <form method="POST" action="{{ route('admin.refundPolicyPost') }}">
                            @csrf
                            ...
                            <br>
                            <label class="control-label">Select Refund Policy Status</label>
                            <div>
                            ...
                            <div class="form-group">
                                <div>
                                    <button type="submit" class="btn btn-success">Add Refund Policy</button>
                                </div>
                            </div>
                            <br>
                        </form>
        ...
        <div id="EditModalForm" class="modal fade">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h1 class="modal-title">Edit Refund Policy</h1>
                    </div>
                    <div class="modal-body">
                        <form method="POST" id="serviceFormEdit" action="">
                            @csrf
                            <input type="hidden" name="refundPolicyId" id="refundPolicyId">
                            <div class="form-group">
                                <label class="control-label">Enter Title</label>
                                <div>
...
{{--                    <th>Description</th>--}}
                ...
                <tbody>
                @foreach($refundPolicy as $refundPol)
                    <tr>
                        <td>{{$refundPol->title}}</td>
{{--                        <td width="100px">{{$refundPol->description}}</td>--}}
                        <td><span class="{{$refundPol->status == 1 ? 'badge badge-success' : 'badge badge-danger'}}">{{$refundPol->status == 1 ? 'Enable' : 'Disable'}}</span></td>
                        <td>
                            <a id="deleteBtn" data-toggle="modal" data-target=".modal1" data-id="{{$refundPol->id}}"
                               class="{{$refundPol->status == 1 ? '' : 'btn btn-sm btn-success'}} delete_btn">{{$refundPol->status == 1 ?  '' : 'Enable'}}</a>
                            <a href="{{url('admin/edit-refund-policy',$refundPol->id)}}" id="serviceEdit" class="btn btn-primary btn-sm"
                               data-id="{{$refundPol->id}}" id="{{$refundPol->id}}" data-toggle="modal" data-target="#EditModalForm">Edit</a>
                        </td>
                    </tr>
                @endforeach
                </tbody>
            ...
            <div class="modal fade modal1" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel">
                <div class="modal-dialog modal-sm" role="document">
                    <div class="modal-content">
                        ...
                        <form method="post" action="{{route('admin.changeRefundPolicyStatus')}}">
                            @csrf
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('body').on('click', '#serviceEdit', function () {
                ...
                $.ajax({
                    type: "GET",
                    url: "{{url('/admin/edit-refund-policy/')}}"+'/'+termsAndConditions_id,
                    success:function (response){
                        console.log(response);
                        $('#refundPolicyId').val(response.id);
                        $('#title').val(response.title);
                        $('#description').val(response.description);
                        $('#status').prop('selectedIndex', response.status);
                        $('#serviceFormEdit').attr('action',"{{url('/admin/edit-refund-policy/')}}"+'/'+termsAndConditions_id);
                    }
                });
            });
        });
